...
const request = async (req, res) => {
    try {
        const { name, phone, insuranceSupport, mechanicServices, pickupDelivery, rentalCar } = req.body;

        const photos = req.files;

        if(!name || !phone || !photos){
        ...
        const requested = await requestEmail(name, phone, insuranceSupport, mechanicServices, pickupDelivery, rentalCar, uploaded);
